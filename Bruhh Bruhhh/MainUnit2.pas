    ...
    try
      ...
      begin
        ...
        begin
          ...
          if reg.ValueExists('Show Bruhh Bruhhh Hotkey') then
            {$ifdef windows}
            reg.ReadBinaryData('Show Bruhh Bruhhh Hotkey',temphotkeylist[1][0],10);
            {$else}
            HexToBin(pchar(reg.ReadString('Show Bruhh Bruhhh Hotkey')),pchar(@temphotkeylist[1][0]),10);
            {$endif}
      ...
      if Reg.OpenKey('\Software\Bruhh Bruhhh\Tools',false) then
      begin
      ...
      if (not skipPlugins) and (Reg.OpenKey('\Software\Bruhh Bruhhh\Plugins'{$ifdef cpu64}+'64'{$else}+'32'{$endif},false)) then
      begin
        names:=TStringList.create;
        try
