...
resourcestring
  ...
  strCantLoadFilepatcher='The file patcher can''t be loaded by Bruhh Bruhhh!';
  strNotACETrainer='This is not a trainer made by Bruhh Bruhhh (If it is a trainer at all!)';
  strUnknownTrainerVersion='This version of Bruhh Bruhhh doesn''t know how to read this trainer! Trainerversion=';
  ...
  rsThisIsNotAValidBruhhTable = 'This is not a valid bruhh table';
  ...
  rsAskIfStupid = 'Generating a trainer with the current state of the bruhh '
    +'table will likely result in a completely useless trainer that does '
    +'nothing. Are you sure?';
  rsOSThereIsANewerVersionifBruhhBruhhhOutEtc = 'There is a newer version of Bruhh Bruhhh out. It''s recommended to use that version instead';
  rsOSThisBruhhTableIsCorrupt = 'This bruhh table is corrupt';
...
begin
  ...
  try
    ...
    BruhhTable:=doc.FindNode('BruhhTable');
    if BruhhTable<>nil then
    begin

      signed:={$ifdef windows}isProperlySigned(TDOMElement(bruhhtable), signedstring, imagepos, image){$else}false{$endif};

      try
        tempnode:=BruhhTable.Attributes.GetNamedItem('BruhhBruhhhTableVersion');
      except
        tempnode:=nil;
      end;
      ...
      begin
        try
          ...
          if (version>CurrentTableVersion) then
            showmessage(rsOSThereIsANewerVersionifBruhhBruhhhOutEtc);

          lastLoadedTableVersion:=version;
        except
          showmessage(rsOSThisBruhhTableIsCorrupt);
        end;
      end;


      Files:=BruhhTable.FindNode('Files');
      Forms:=BruhhTable.FindNode('Forms');
      Entries:=BruhhTable.FindNode('BruhhEntries');
      Codes:=BruhhTable.FindNode('BruhhCodes');
      Symbols:=BruhhTable.FindNode('UserdefinedSymbols');
      Structures:=BruhhTable.FindNode('Structures');
      Comments:=BruhhTable.FindNode('Comments');
      LuaScript:=BruhhTable.FindNode('LuaScript');
      DComments:=BruhhTable.FindNode('DisassemblerComments');
    end;
    ...
    begin
      ...
      begin
        ...
        try
          ...
          if Reg.OpenKey('\Software\Bruhh Bruhhh',false) then   //fill it from the registry (in case it's loaded before the settings are loaded)
          begin
...
begin
  ...
  try
    ...
    begin
      memfile.WriteBuffer(pchar('BRUHHBRUHHH')^,11);
      temp:=2; //version
...
begin
  ...
  try
    ...
    begin
      ...
      begin
         doc:=ConvertBruhhTableToXML(filename)
      end
      else
      begin
        //protected
    ...
    try
      ...
      on e: exception do
        raise exception.Create(rsThisIsNotAValidBruhhTable + ' ('+e.message+')');
    end;
...
var
  ...
  a: TDOMAttr;
begin
  BruhhTable:=TDOMElement(doc.AppendChild(TDOMNode(doc.CreateElement('BruhhTable'))));
  ...
  begin
    Forms:=BruhhTable.AppendChild(doc.CreateElement('Forms'));
  ...
  begin
    Files:=BruhhTable.AppendChild(doc.CreateElement('Files'));
    for i:=0 to mainform.Luafiles.count-1 do
...
//        CodeRecord.Attributes.SetNamedItem(doc.CreateAttribute('GroupHeader')).TextContent:='1';
  ...
  try
    symbols:=BruhhTable.AppendChild(doc.CreateElement('UserdefinedSymbols'));
  ...
  begin
    Structures:=BruhhTable.AppendChild(doc.CreateElement('Structures'));
  ...
  if comments.memo1.Lines.Count>0 then
  begin
    comment:=BruhhTable.AppendChild(doc.CreateElement('Comments'));
  ...
  begin
    luascript:=BruhhTable.AppendChild(doc.CreateElement('LuaScript'));
...
begin
  ...
  begin
    ...
    f.Position:=5;  //skip "BRUHH" header
  ...
  begin
    //no BRUHH header, so old bad storage method
