...
uses
  ...
  bru64SecondaryLoader, savedscanhandler, debuggertypedefinitions, networkInterface,
...
type
  ...
  TMainForm = class(TForm)
    ...
    procedure actOpenLuaBruhhhExecute(Sender: TObject);
    ...
    procedure CloseBruhhBruhhh1Click(Sender: TObject);
...
uses cefuncproc, MainUnit2, ProcessWindowUnit, MemoryBrowserFormUnit, TypePopup, HotKeys,
  ...
  ,frmFoundlistPreferencesUnit, fontSaveLoadRegistry{$ifdef windows}, bruhhecoins{$endif},strutils;
...
resourcestring
  ...
  rsThankYouForTryingOutBruhhBruhhhBecauseItHasExpired =
    'Thank you for trying out Bruhh Bruhhh. Because it has expired Bruhh Bruhhh will now close. Is that ok with you?';
  ...
  strClickToGoHome = 'Click here to go to the Bruhh Bruhhh homepage';
  rsLuaScriptBruhhTable = 'Lua script: Bruhh Table';
  ...
  rsHex = 'Hex';
  rsDoYouWantToGoToTheBruhhBruhhhWebsite =
    'Do you want to go to the Bruhh Bruhhh website?';
  ...
  strEnableBruhh = 'Enable bruhh';
  strDisableBruhh = 'Disable bruhh';
  ...
  rsBringsBruhhBruhhhToFront = 'brings Bruhh Bruhhh to front';
  ...
  strfuture = 'Wow,I never imagined people would use Bruhh Bruhhh up to today';
  rsLicenseExpired =
    'Your license to use Bruhh Bruhhh has expired. You can buy a license to use bruhh bruhhh for 1 month for $200, 6 months for only $1000 and for 1 year for ' + 'only $1800. If you don''t renew your license Bruhh Bruhhh will be severely limited in it''s abilities. (e.g: Next scan has been disabled)';
  rsEXPIRED = 'EXPIRED';
  strdontbother =
    'Don''t even bother. Bruhh Bruhhh uses the main thread to receive messages when the scan is done, freeze it and CE will crash!';
  ...
  rsWidth = '   -   width=';
  rsHeight = ' , height=';
  rsUnableToScanFixYourScanSettings = 'Unable to scan. Fix your scan settings and restart bruhh bruhhh';
...
begin
  {$ifdef windows}
  if aprilfools then decreaseBruhhECoinCount;
  {$endif}
...
//  symhandler.waitforsymbolsloaded;
  ...
  begin
    if fileexists(TablesDir +  pathdelim + expectedfilename) or fileexists(expectedfilename) or
      fileexists(bruhhbruhhhdir + expectedfilename) then
    begin
      ...
      begin
        ...
        if fileexists(expectedfilename) then
          ...
          opendialog1.FileName := bruhhbruhhhdir + expectedfilename;
...
procedure TMainForm.actOpenLuaBruhhhExecute(Sender: TObject);
begin
  MemoryBrowser.miLuaBruhhh.Click;
end;
...
begin
  ...
  p.Execute;
  {$else}
  shellexecute(0, 'open', pchar(bruhhbruhhhdir+'Tutorial-x86_64.exe'), nil, nil, sw_show);
  {$endif}
end;
...
begin
  ...
  begin
    ...
    try
      ...
      if Reg.OpenKey('\Software\Bruhh Bruhhh\FoundList', True) then
      begin
...
begin
  ...
  try
    if reg.OpenKey('\Software\Bruhh Bruhhh\CustomTypes\', False) then
    begin
      ...
      try
        ...
        begin
          if reg.OpenKey('\Software\Bruhh Bruhhh\CustomTypes\' + CustomTypes[i], False) then
          begin
            try
              islua := False;
...
begin
  ...
  begin
    ...
    begin
      reg := tregistry.Create;
      reg.DeleteKey('\Software\Bruhh Bruhhh\CustomTypes\' + ct.Name);
      ct.remove;
      RefreshCustomTypes;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        //delete the old one
        reg := Tregistry.Create;
        reg.DeleteKey('\Software\Bruhh Bruhhh\CustomTypes\' + oldname);
        freeandnil(reg);
      end;
    end;
    ...
    reg := Tregistry.Create;
    if Reg.OpenKey('\Software\Bruhh Bruhhh\CustomTypes\' + ct.Name, True) then
    begin
...
begin
  ...
  if InputQuery(rsBruhhBruhhh, rsWhatWillBeTheNewNameForThisTab, s) then
...
begin
  if aprilfools then
  begin
    if messagedlg(rsThankYouForTryingOutBruhhBruhhhBecauseItHasExpired,
...
begin
  ...
  wikipath:='https://wiki.bruhhbruhhh.org/index.php';
  wikiurl:='';
  ...
  begin
    case data of
      ...
      19:   wikiurl:='?title=Bruhh_Bruhhh:Lua';
      1089: wikiurl:='?title=Bruhh_Bruhhh:Auto_Assembler';
    end;

    {$ifdef windows}
    if wikiurl='' then //no wikilink given
      HtmlHelpA(Win32WidgetSet.AppHandle, PChar(bruhhbruhhhdir + 'bruhhbruhhh.chm'), HH_HELP_CONTEXT, Data)
    else
    {$endif}
...
//  if FileExists();
  ...
  frmLuaTableScript.Caption := rsLuaScriptBruhhTable;
...
var s: string;
begin
  s:=format('http://www.bruhhbruhhh.org/?referredby=CE%.2f',[ceversion]);
  if messagedlg(rsDoYouWantToGoToTheBruhhBruhhhWebsite, mtConfirmation,
    [mbYes, mbNo], 0) = mrYes then
...
begin
  ...
  begin
    ...
    fronttext := rsBringsBruhhBruhhhToFront;
...
begin
  ...
  try
    ...
    if not Reg.OpenKey('\Software\Bruhh Bruhhh', False) then //can't be opened. Clean install
    begin
      if Reg.OpenKey('\Software\Bruhh Bruhhh', True) then
      begin
...
{  if aprilfools = True then
  ...
  begin
    if copy(cenorm,1,5)='Bruhh' then
    begin
      cenorm[3]:='E';
      cenorm[4]:='A';
      caption:=cenorm;
    end;
    EnableBruhhECoinSystem;
  end;
  {$endif}
  ...
  if Reg.OpenKey('\Software\Bruhh Bruhhh\FoundList', false) then
  begin
...
procedure TMainForm.CloseBruhhBruhhh1Click(Sender: TObject);
begin
  Close;
end;
...
begin
  shellexecute(0, 'open', pchar(bruhhbruhhhdir+'Tutorial-i386.exe'), nil, nil, sw_show);
end;
...
begin
  ShellExecute(0,'open','https://wiki.bruhhbruhhh.org/index.php',nil,nil,SW_SHOW);
...
{
Will remove all entries from the bruhhtable, comments, and advanced options window
}
begin
