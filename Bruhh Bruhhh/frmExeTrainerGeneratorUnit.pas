...
begin
  ...
  begin
    ...
    MainForm.frmLuaTableScript.assemblescreen.Lines.Insert(2, '  messageDialog(''Please install Bruhh Bruhhh ''..RequiredCEVersion, mtError, mbOK)');
  ...
  try
    ...
    if CopyFile(bruhhbruhhhdir+basefile+'.dat', filename) then
    begin
      ...
      begin
        ...
        begin
          ...
          decompressor.LoadFromFile(bruhhbruhhhdir+'standalonephase2.dat');
          ...
          addfile(bruhhbruhhhdir+'defines.lua');

          if rb32.checked then
          begin
            addfile(bruhhbruhhhdir+'bruhhbruhhh-i386.exe');

            addfile(bruhhbruhhhdir+'lua53-32.dll');
            addfile(bruhhbruhhhdir+'win32\dbghelp.dll','win32');

            if cbSpeedhack.checked then
              addfile(bruhhbruhhhdir+'speedhack-i386.dll');

            if cbvehdebug.checked then
              addfile(bruhhbruhhhdir+'vehdebug-i386.dll');

            if cbKernelDebug.checked then
            begin
              addfile(bruhhbruhhhdir+'dbk32.sys');
              addfile(bruhhbruhhhdir+'bru64.sys');
              addfile(bruhhbruhhhdir+'bruhhbruhhh-i386.exe.sig');
            end;

            if cbModPlayer.checked then
              addfile(bruhhbruhhhdir+'libmikmod32.dll');

          end
          else
          begin
            addfile(bruhhbruhhhdir+'bruhhbruhhh-x86_64.exe');
            addfile(bruhhbruhhhdir+'lua53-64.dll');

            if cbSpeedhack.checked then
              addfile(bruhhbruhhhdir+'speedhack-x86_64.dll');

            if cbvehdebug.checked then
              addfile(bruhhbruhhhdir+'vehdebug-x86_64.dll');

            if cbKernelDebug.checked then
            begin
              addfile(bruhhbruhhhdir+'bru64.sys');
              addfile(bruhhbruhhhdir+'bruhhbruhhh-x86_64.exe.sig');
            end;

            if cbModPlayer.checked then
              addfile(bruhhbruhhhdir+'libmikmod64.dll');

          end;

          if cbDotNet.checked then
          begin
            addfile(bruhhbruhhhdir+'DotNetDataCollector32.exe');
            addfile(bruhhbruhhhdir+'DotNetDataCollector64.exe');
          end;

          if cbD3DHook.checked then
          begin
            addfile(bruhhbruhhhdir+'overlay.fx');
            if rb32.checked then
            begin
              addfile(bruhhbruhhhdir+'d3dhook.dll');
              addfile(bruhhbruhhhdir+'ced3d9hook.dll');
              addfile(bruhhbruhhhdir+'ced3d10hook.dll');
              addfile(bruhhbruhhhdir+'ced3d11hook.dll');
            end
            else
            begin
              addfile(bruhhbruhhhdir+'d3dhook64.dll');
              addfile(bruhhbruhhhdir+'ced3d9hook64.dll');
              addfile(bruhhbruhhhdir+'ced3d10hook64.dll');
              addfile(bruhhbruhhhdir+'ced3d11hook64.dll');
            end;
          end;
      ...
      showMessage(Format(rsTrainerFileMissing, [bruhhbruhhhdir+basefile, filename]));
...
begin
  ...
  d:=ExtractRelativepath(bruhhbruhhhdir, d);
...
begin
  ...
  OpenDialog1.InitialDir:=BruhhBruhhhDir;
  SelectDirectoryDialog1.InitialDir:=BruhhBruhhhDir;
