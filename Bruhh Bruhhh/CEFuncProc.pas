...
//Bruhh Bruhhh regular WONT look at this
...
begin
  assignfile(memoryfile,BruhhBruhhhDir+'Memory.TMP');
  assignfile(addressfile,BruhhBruhhhDir+'Addresses.TMP');
  reset(memoryfile,1);
  reset(addressfile,1);

  assignfile(newmemoryfile,BruhhBruhhhDir+'Memory2.TMP');
  assignfile(newaddressfile,BruhhBruhhhDir+'Address2.TMP');
  ...
  deletefile(BruhhBruhhhDir+'Memory.UNDO');
  deletefile(BruhhBruhhhDir+'Addresses.UNDO');
  renamefile(BruhhBruhhhDir+'Memory.tmp',bruhhbruhhhdir+'Memory.UNDO');
  renamefile(BruhhBruhhhDir+'Addresses.tmp',BruhhBruhhhDir+'Addresses.UNDO');
  renamefile(BruhhBruhhhDir+'Memory2.tmp',BruhhBruhhhDir+'Memory.TMP');
  Renamefile(BruhhbruhhhDir+'Address2.TMP',BruhhBruhhhDir+'Addresses.TMP');
...
//http://forum.bruhhbruhhh.org/viewtopic.php?t=564382
{$IFDEF windows}
type
...
{$ENDIF}
begin
  BruhhBruhhhDir:=ExtractFilePath(application.ExeName);
  result:=BruhhBruhhhDir;
  ...
  if SHGetPathFromIDList(PIDL, Path) then
    tablesdir := WinCPToUTF8(Path)+'\My Bruhh Tables';
  SHGetMalloc(AMalloc);
  AMalloc.Free(PIDL);
  StrDispose(Path);
...
begin
  deletefile(BruhhBruhhhDir+'Memory.TMP');
  deletefile(BruhhBruhhhDir+'Addresses.TMP');
  deletefile(BruhhBruhhhDir+'Memory.UNDO');
  deletefile(BruhhBruhhhDir+'Addresses.UNDO');
  freemem(memory);
  memory:=nil;
...
begin
  ...
  try
    ...
    try
      ...
      if Reg.OpenKey('\Software\Bruhh Bruhhh',false) then
      begin
        if reg.valueexists('Save window positions') then
          if reg.readbool('Save window positions') = false then exit;
      end;

      if Reg.OpenKey('\Software\Bruhh Bruhhh\Window Positions '+inttostr(screen.PixelsPerInch),false) or Reg.OpenKey('\Software\Bruhh Bruhhh\Window Positions',false) then
      begin
        s:=form.Name;
        s:=s+rsPosition;
...
begin
  ...
  begin
    ...
    try
      ...
      if Reg.OpenKey('\Software\Bruhh Bruhhh\Window Positions '+inttostr(screen.PixelsPerInch),true) then
      begin
...
begin
  result:=filename;
  if pos(uppercase(BruhhBruhhhDir),uppercase(filename))=1 then
    result:='.\'+copy(filename,length(BruhhBruhhhdir)+1,length(filename));
end;
...
begin
  ...
  path:=path+'Bruhh Bruhhh Symbols';
