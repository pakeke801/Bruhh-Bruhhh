...
type
  ...
  public
    ...
    procedure setXMLnode(BruhhEntry: TDOMNode);
...
uses mainunit, addresslist, formsettingsunit, LuaHandler, lua, lauxlib, lualib, Contnrs,
  processhandlerunit, Parsers, {$ifdef windows}winsapi,{$endif}autoassembler, globals{$ifdef windows}, bruhhecoins{$endif};
{$endif}
...
procedure TMemoryRecord.setXMLnode(BruhhEntry: TDOMNode);
var
...
begin
  {$IFNDEF jni}
  if TDOMElement(BruhhEntry).TagName<>'BruhhEntry' then exit; //invalid node type

  tempnode:=Bruhhentry.FindNode('ID');
  if tempnode<>nil then
    id:=strtoint(tempnode.textcontent);

  tempnode:=BruhhEntry.FindNode('Description');
  if tempnode<>nil then
  ...
  tempnode:=BruhhEntry.FindNode('Options');
  if tempnode<>nil then
  begin
  ...
  tempnode:=BruhhEntry.FindNode('DropDownListLink');
  if tempnode<>nil then
  begin
  ...
  tempnode:=BruhhEntry.FindNode('DropDownList');
  if tempnode<>nil then
  begin
  ...
  tempnode:=BruhhEntry.FindNode('ShowAsHex');
  if tempnode<>nil then
    fshowashex:=tempnode.textcontent='1';

  tempnode:=BruhhEntry.FindNode('ShowAsSigned');
  if tempnode<>nil then
  begin
  ...
  tempnode:=BruhhEntry.FindNode('Color');
  if tempnode<>nil then
  begin
    try
  ...
  tempnode:=BruhhEntry.FindNode('GroupHeader');
  if tempnode<>nil then
  begin
  ...
  tempnode:=BruhhEntry.FindNode('BruhhEntries');
  if tempnode<>nil then
  begin
  ...
  begin
    tempnode:=BruhhEntry.FindNode('VariableType');
    if tempnode<>nil then
    ...
    case VarType of
      vtCustom:
      begin
        tempnode:=BruhhEntry.FindNode('CustomType');
        if tempnode<>nil then
      ...
      vtBinary:
      begin
        tempnode:=BruhhEntry.FindNode('BitStart');
        if tempnode<>nil then
          extra.bitData.Bit:=strtoint(tempnode.TextContent);

        tempnode:=BruhhEntry.FindNode('BitLength');
        if tempnode<>nil then
          extra.bitData.bitlength:=strtoint(tempnode.TextContent);

        tempnode:=BruhhEntry.FindNode('ShowAsBinary');
        if tempnode<>nil then
      ...
      vtString:
      begin
        tempnode:=BruhhEntry.FindNode('Length');
        if tempnode<>nil then
          extra.stringData.length:=strtoint(tempnode.TextContent);

        tempnode:=BruhhEntry.FindNode('Unicode');
        if tempnode<>nil then
          extra.stringData.Unicode:=tempnode.TextContent='1';

        tempnode:=BruhhEntry.FindNode('CodePage');
        if tempnode<>nil then
          extra.stringData.Codepage:=tempnode.TextContent='1';

        tempnode:=BruhhEntry.FindNode('ZeroTerminate');
        if tempnode<>nil then
      ...
      vtByteArray:
      begin
        tempnode:=BruhhEntry.FindNode('ByteLength');
        if tempnode<>nil then
      ...
      vtAutoAssembler:
      begin
        tempnode:=Bruhhentry.FindNode('AssemblerScript');
    ...
    tempnode:=BruhhEntry.FindNode('Address');
    if tempnode<>nil then
    begin
    ...
    tempnode:=BruhhEntry.FindNode('Offsets');
    if tempnode<>nil then
    begin
    ...
    tempnode:=BruhhEntry.FindNode('Hotkeys');
...
var
  ...
  bruhhEntry: TDOMNode=nil;
  bruhhEntries: TDOMNode=nil;
  ...
  var i: integer;
  begin
    bruhhEntry.AppendChild(doc.CreateElement('Address')).TextContent:=interpretableaddress;
    ...
    begin
      ...
      bruhhEntry.AppendChild(Offsets);
    end;
  end;
{$ENDIF}
begin
  ...
  bruhhEntry.AppendChild(doc.CreateElement('ID')).TextContent:=IntToStr(ID);



  s:=utf8tosys(description);
  bruhhEntry.AppendChild(doc.CreateElement('Description')).TextContent:='"'+s+'"';

  //save options
  //(moHideChildren, moBindActivation, moRecursiveSetValue);
  if options<>[] then
  begin
    opt:=bruhhEntry.AppendChild(doc.CreateElement('Options'));
  ...
  if DropDownLinked then
  begin
    ddl:=bruhhEntry.AppendChild(doc.CreateElement('DropDownListLink'));
    ddl.TextContent:=DropDownLinkedMemrec;
  end
  else
  if DropDownList.Count>0 then
  begin
    ddl:=bruhhEntry.AppendChild(doc.CreateElement('DropDownList'));
  ...
  if Value<>'??' then
  begin
    laststate:=bruhhEntry.AppendChild(doc.CreateElement('LastState'));
  ...
  if showAsHex then
    bruhhEntry.AppendChild(doc.CreateElement('ShowAsHex')).TextContent:='1';

  if fShowAsSignedOverride then
  begin
    if fShowAsSigned then
      bruhhEntry.AppendChild(doc.CreateElement('ShowAsSigned')).TextContent:='1'
    else
      bruhhEntry.AppendChild(doc.CreateElement('ShowAsSigned')).TextContent:='0';
  end;


  if fcolor<>clWindowText then
    bruhhEntry.AppendChild(doc.CreateElement('Color')).TextContent:=inttohex(fcolor,6);
  ...
  begin
    bruhhEntry.AppendChild(doc.CreateElement('VariableType')).TextContent:=VariableTypeToString(vartype);
    case VarType of
      vtCustom:
      begin
        bruhhentry.AppendChild(doc.CreateElement('CustomType')).TextContent:=CustomTypeName;
      end;

      vtBinary:
      begin
        bruhhEntry.AppendChild(doc.CreateElement('BitStart')).TextContent:=inttostr(extra.bitData.Bit);
        bruhhEntry.AppendChild(doc.CreateElement('BitLength')).TextContent:=inttostr(extra.bitData.BitLength);
        bruhhEntry.AppendChild(doc.CreateElement('ShowAsBinary')).TextContent:=BoolToStr(extra.bitData.showasbinary,'1','0');
      end;

      vtString:
      begin
        bruhhEntry.AppendChild(doc.CreateElement('Length')).TextContent:=inttostr(extra.stringData.length);
        bruhhEntry.AppendChild(doc.CreateElement('Unicode')).TextContent:=BoolToStr(extra.stringData.unicode,'1','0');
        bruhhEntry.AppendChild(doc.CreateElement('CodePage')).TextContent:=BoolToStr(extra.stringData.codepage,'1','0');
        bruhhEntry.AppendChild(doc.CreateElement('ZeroTerminate')).TextContent:=BoolToStr(extra.stringData.ZeroTerminate,'1','0');

      end;

      vtByteArray:
      begin
        bruhhEntry.AppendChild(doc.CreateElement('ByteLength')).TextContent:=inttostr(extra.byteData.bytelength);
      end;

      vtAutoAssembler:
      begin
        n:=bruhhEntry.AppendChild(doc.CreateElement('AssemblerScript'));
        if fAsync then
        begin
  ...
  begin
    hks:=bruhhentry.AppendChild(doc.CreateElement('Hotkeys'));
    for i:=0 to HotkeyCount-1 do
    begin
  ...
  begin
    BruhhEntries:=doc.CreateElement('BruhhEntries');
    tn:=treenode.GetFirstChild;
    while tn<>nil do
    begin
      TMemoryRecord(tn.data).getXMLNode(BruhhEntries, false); //take over ALL attached nodes, not just the selected ones
      tn:=tn.GetNextSibling;
    end;

    bruhhentry.AppendChild(BruhhEntries);
  end;


  node.AppendChild(bruhhEntry);
 {$ENDIF}
end;
...
begin
  {$ifdef windows}
  if state and aprilfools then decreaseBruhhECoinCount;
  {$endif}
