...
procedure ReloadAllLuaBruhhhHighlighters;

var
  frmLuaBruhhh: TfrmLuaBruhhh;
...
resourcestring
  ...
  rsLuaBruhhh = 'Lua Bruhhh';

var
  LuaDebugForm: TfrmLuaBruhhh;
...
var
  i: integer;
  f: TCustomForm;
  lef: TfrmLuaBruhhh absolute f;
begin
  for i:=0 to screen.FormCount-1 do
  begin
    f:=screen.Forms[i];
    if f is TfrmLuaBruhhh then
      lef.reloadHighlighterSettings;
  end;

end;

procedure TfrmLuaBruhhh.Panel2Resize(Sender: TObject);
begin
  btnexecute.Height:=panel2.clientheight-(2*btnexecute.top);
end;

procedure TfrmLuaBruhhh.ContinueAutoComplete;
var p,p2: tpoint;
begin
...
procedure TfrmLuaBruhhh.ContinueAutoComplete2(sender: TObject);
begin
...
procedure TfrmLuaBruhhh.scLuaCompleterCodeCompletion(var Value: string;
...
procedure TfrmLuaBruhhh.scLuaCompleterExecute(Sender: TObject);
var
  s,s2,extra: string;
  w: tpoint;
  i,j,si: integer;
...
procedure TfrmLuaBruhhh.scLuaCompleterKeyCompletePrefix(Sender: TObject);
begin

end;

procedure TfrmLuaBruhhh.scLuaCompleterPositionChanged(Sender: TObject);
begin

end;

procedure TfrmLuaBruhhh.scLuaCompleterSearchPosition(var APosition: integer);
var
  s,s2: string;
  i,p: integer;
  start: integer;
...
procedure TfrmLuaBruhhh.SQLConnector1AfterConnect(Sender: TObject);
begin

end;

procedure TfrmLuaBruhhh.tbRunClick(Sender: TObject);
begin
...
procedure TfrmLuaBruhhh.tbSingleStepClick(Sender: TObject);
begin
...
procedure TfrmLuaBruhhh.tbStopDebugClick(Sender: TObject);
begin
...
procedure TfrmLuaBruhhh.tShowHintTimer(Sender: TObject);
var r: trect;
  description: string;
  p,p2,p3: tpoint;
  token: string;
...
begin
  ...
  begin
    ...
    begin
      //break
     // frmLuaBruhhh.visible:=false;
     // frmLuaBruhhh.ShowModal;
     // frmLuaBruhhh.show;
...
//    frmLuaBruhhh.moutput.lines.add('called:'+ar.what+' ('+inttostr(ar.currentline)+')');
...
procedure TfrmLuaBruhhh.btnExecuteClick(Sender: TObject);
var pc: pchar;
  i,j,ln: integer;
...
procedure TfrmLuaBruhhh.cbShowOnPrintClick(Sender: TObject);
begin

end;

procedure TfrmLuaBruhhh.dlgReplaceFind(Sender: TObject);
...
procedure TfrmLuaBruhhh.dlgReplaceReplace(Sender: TObject);
var so: TSynSearchOptions;
begin
  so:=[];
  if not (frDown in dlgReplace.Options) then
...
procedure TfrmLuaBruhhh.FormCreate(Sender: TObject);
var
  x: array of integer;
  fq: TFontQuality;
  i: integer;
...
procedure TfrmLuaBruhhh.FormDestroy(Sender: TObject);
...
procedure TfrmLuaBruhhh.FormShow(Sender: TObject);
var i, off: integer;
begin
...
procedure TfrmLuaBruhhh.miUndoClick(Sender: TObject);
begin
  mscript.Undo;
end;

procedure TfrmLuaBruhhh.MenuItem11Click(Sender: TObject);
var f: TfrmLuaBruhhh;
begin
  f:=TfrmLuaBruhhh.create(application);
  f.miView.visible:=false;

  f.show;
end;

procedure TfrmLuaBruhhh.miFindClick(Sender: TObject);
begin
  finddialog1.Execute;
end;

procedure TfrmLuaBruhhh.miRedoClick(Sender: TObject);
begin
  mscript.redo;
end;

procedure TfrmLuaBruhhh.reloadHighlighterSettings;
begin
  synhighlighter.LoadFromRegistry(HKEY_CURRENT_USER, '\Software\Bruhh Bruhhh\Lua Highlighter');
end;

procedure TfrmLuaBruhhh.MenuItem15Click(Sender: TObject);
var
...
begin
  ...
  begin
    synhighlighter.SaveToRegistry(HKEY_CURRENT_USER, '\Software\Bruhh Bruhhh\Lua Highlighter');
    ReloadAllAutoInjectHighlighters; //AA uses lua too
    ReloadAllLuaBruhhhHighlighters;
  end;

  frmHighlighterEditor.free;
end;

procedure TfrmLuaBruhhh.MenuItem2Click(Sender: TObject);
begin
...
procedure TfrmLuaBruhhh.MenuItem3Click(Sender: TObject);
begin
...
procedure TfrmLuaBruhhh.MenuItem5Click(Sender: TObject);
begin
  moutput.Clear;
end;

procedure TfrmLuaBruhhh.miFindReplaceClick(Sender: TObject);
begin
  dlgReplace.Execute;
end;

procedure TfrmLuaBruhhh.miCutClick(Sender: TObject);
begin
  mscript.CutToClipboard;
end;

procedure TfrmLuaBruhhh.miCopyClick(Sender: TObject);
begin
  mscript.CopyToClipboard;
end;

procedure TfrmLuaBruhhh.miPasteClick(Sender: TObject);
begin
  mscript.PasteFromClipboard;
end;

procedure TfrmLuaBruhhh.miResizeOutputClick(Sender: TObject);
begin
...
procedure TfrmLuaBruhhh.miSaveCurrentScriptAsClick(Sender: TObject);
begin
  if savedialog1.Execute then
  begin
    mscript.lines.SaveToFile(savedialog1.filename);
    frmLuaBruhhh.Caption:=rsLuaBruhhh+' '+ExtractFileNameOnly(savedialog1.filename);
  end;
end;

procedure TfrmLuaBruhhh.miSetBreakpointClick(Sender: TObject);
begin
  mScriptGutterClick(mScript, 0,0, mscript.CaretY, nil);
end;

procedure TfrmLuaBruhhh.miShowScriptInOutputClick(Sender: TObject);
begin

end;

procedure TfrmLuaBruhhh.mScriptChange(Sender: TObject);
begin

end;

procedure TfrmLuaBruhhh.mScriptGutterClick(Sender: TObject; X, Y,
...
procedure TfrmLuaBruhhh.mScriptKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmLuaBruhhh.mScriptKeyPress(Sender: TObject; var Key: char);
var p,p2: tpoint;
begin
...
procedure TfrmLuaBruhhh.mScriptMouseEnter(Sender: TObject);
begin
  tShowHint.enabled:=false;
  tShowHint.AutoEnabled:=true;
end;

procedure TfrmLuaBruhhh.mScriptMouseLeave(Sender: TObject);
begin
  tShowHint.AutoEnabled:=false;
  tShowHint.enabled:=false;
end;

procedure TfrmLuaBruhhh.mScriptMouseLink(Sender: TObject; X, Y: Integer;
  var AllowMouseLink: Boolean);
begin
  AllowMouseLink:=false;
end;

procedure TfrmLuaBruhhh.mScriptMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
  if hintwindow<>nil then
    hintwindow.hide;
...
procedure TfrmLuaBruhhh.mScriptShowHint(Sender: TObject; HintInfo: PHintInfo);
begin
