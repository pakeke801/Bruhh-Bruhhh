...
type
  ...
  TMemoryBrowser = class(TForm)
    ...
    procedure miLuaBruhhhClick(Sender: TObject);
...
uses Valuechange, MainUnit, debugeventhandler, findwindowunit,
  ...
  frmluabruhhhunit, disassemblerviewlinesunit, frmBreakpointConditionunit,
...
resourcestring
  ...
  rsBruhhBruhhhSingleLingeAssembler = 'Single-line assembler';
  ...
  rsPleaseTargetAnotherProcess = 'Start another version of Bruhh Bruhhh and attach to that instead';
...
procedure TMemoryBrowser.miLuaBruhhhClick(Sender: TObject);
begin
  //start lua bruhhh window
  if frmLuaBruhhh=nil then
    frmLuaBruhhh:=TfrmLuaBruhhh.create(MemoryBrowser); //main mb
...
begin
  ...
  try
    if reg.OpenKey('\Software\Bruhh Bruhhh\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\',true) then
    begin
    ...
    if reg.OpenKey('\Software\Bruhh Bruhhh\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\Font',true) then
      SaveFontToRegistry(disassemblerview.font, reg);

    if reg.OpenKey('\Software\Bruhh Bruhhh\Hexview '+inttostr(screen.PixelsPerInch),true) then
    begin
      reg.writeInteger('spaceBetweenLines', hexview.spaceBetweenLines);
      reg.WriteBool('showStatusBar', hexview.statusbar.Visible);
    end;

    if reg.OpenKey('\Software\Bruhh Bruhhh\Hexview '+inttostr(screen.PixelsPerInch)+'\Font',true) then
      SaveFontToRegistry(hexview.hexfont, reg);

    if reg.OpenKey('\Software\Bruhh Bruhhh\RegisterView '+inttostr(screen.PixelsPerInch)+'\Font',true) then
...
begin
  ...
  reg:=Tregistry.Create;
  try
    if reg.OpenKey('\Software\Bruhh Bruhhh\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\Font',false) then
    begin
      LoadFontFromRegistry(f, reg);
      disassemblerview.font:=f;
    end;

    if reg.OpenKey('\Software\Bruhh Bruhhh\Disassemblerview '+inttostr(screen.PixelsPerInch)+'\',false) then
    begin
    ...
    if reg.OpenKey('\Software\Bruhh Bruhhh\Hexview '+inttostr(screen.PixelsPerInch),false) then
    begin
    ...
    if reg.OpenKey('\Software\Bruhh Bruhhh\Hexview '+inttostr(screen.PixelsPerInch)+'\Font',false) then
    begin
      LoadFontFromRegistry(f, reg);
      hexview.hexfont:=f;
    end;

    if reg.OpenKey('\Software\Bruhh Bruhhh\RegisterView '+inttostr(screen.PixelsPerInch)+'\Font',false) then
    begin
      LoadFontFromRegistry(f, reg);
//      scrollbox1.Font:=f;
...
//this will add the selected recdord to the list
  hexview.AddSelectedAddressToBruhhTable;
...
begin
  ...
  begin
    ...
    begin
      beep; //Best sound effect bruhh bruhhh has
      exit;
    end;
  end;
...
//  copy
  ...
  assemblercode:=InputboxTop(rsBruhhBruhhhSingleLingeAssembler, Format(rsTypeYourAssemblerCodeHereAddress, [inttohex(Address, 8)]), assemblercode, x='', canceled{$ifndef darwin},assemblerHistory{$endif});
...
{will save a bruhh bruhhh memory region file .CEM}
begin
