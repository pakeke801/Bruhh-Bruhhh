...
type
  TLuaSettings=class    //A wrapper for the registry object to make access to the bruhh bruhhh settings easier and uniform
  private
    freg: Tregistry;
    fpath: string;
...
begin
  ...
  if freg.OpenKey('\Software\Bruhh Bruhhh\'+v, true) then
    fpath:=v;
end;
...
begin
  ...
  if initialpath<>nil then
    ...
    freg.OpenKey('\Software\Bruhh Bruhhh',true);
