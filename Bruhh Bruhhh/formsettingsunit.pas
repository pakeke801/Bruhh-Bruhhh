...
resourcestring
  ...
  rsYouHavenTSelectedAnyMemoryTypeThisWillResultInChea = 'You haven''t selected any memory type. This will result in Bruhh Bruhhh finding NO memory! Are you stupid?';
  ...
  rsPopupHideBruhhBruhhh = 'Popup/Hide bruhh bruhhh';
  ...
  rsRestartCE = 'It is recommended to restart Bruhh Bruhhh for this change to take effect';
...
begin
    ...
    try
      ...
      begin
        ...
        try
          ...
          reg.{$ifdef windows}WriteBinaryData{$else}WriteString{$endif}('Show Bruhh Bruhhh Hotkey',{$ifndef windows}bintohexs({$endif}frameHotkeyConfig.newhotkeys[1][0],10){$ifndef windows}){$endif};
        ...
        begin
          if reg.OpenKey('\Software\Bruhh Bruhhh\Font', true) then
  ...
  {$ifndef net}
      ...
      reg.DeleteKey('\Software\Bruhh Bruhhh\Tools');
      if Reg.OpenKey('\Software\Bruhh Bruhhh\Tools',true) then
      begin
      ...
      //save the plugins
      reg.DeleteKey('\Software\Bruhh Bruhhh\Plugins'+cpu);
      if Reg.OpenKey('\Software\Bruhh Bruhhh\Plugins'+cpu,true) then
      begin
...
begin
  ...
  begin
    ...
    try
      ini:=TIniFile.Create(bruhhbruhhhdir+'languages' + DirectorySeparator+'language.ini');
      try
...
begin
  ...
  FindAllDirectories(f,BruhhBruhhhDir+'\languages',false);
  for i:=0 to f.Count-1 do
  begin
    n:=f[i];
    if not (fileexists(n+'\bruhhbruhhh.po') or fileexists(n+'\bruhhbruhhh-x86_64.po') or fileexists(n+'\bruhhbruhhh-i386.po')) then
      continue;
