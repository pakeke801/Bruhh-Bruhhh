...
type
  ...
  private
   ...
   // procedure TreeviewKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    ...
    function BruhhTableNodeHasOnlyAutoAssemblerScripts(BruhhTable: TDOMNode): boolean; //helperfunction
    procedure BruhhTableNodeCheckForRelativeAddress(BruhhTable: TDOMNode; var hasRelative, allRelative: boolean); //helperprocedure
  ...
  public
    ...
    procedure refresh;
    procedure SelectAll;
    procedure saveTableXMLToNode(BruhhEntries: TDOMNode; selectedOnly: boolean=false);
    procedure loadTableXMLFromNode(BruhhEntries: TDOMNode);
...
procedure TAddresslist.saveTableXMLToNode(BruhhEntries: TDOMNode; selectedOnly: boolean=false);
var tn: TTreenode;
p: TTreenode;
m: TMemoryRecord;
begin
  ...
  while tn<>nil do
  begin
    TMemoryRecord(tn.data).getXMLNode(BruhhEntries,selectedonly);
...
procedure TAddresslist.loadTableXMLFromNode(BruhhEntries: TDOMNode);
var currentEntry: TDOMNode;
memrec: TMemoryRecord;
i: integer;
begin

  currentEntry:=BruhhEntries.FirstChild;
  while currententry<>nil do
  begin
    if tdomelement(currententry).TagName='BruhhEntry' then
    begin
...
var
  doc: TXMLDocument;
  bruhhtable,BruhhEntries: TDOMNode;
  i: integer;
  ms: TMemoryStream;
begin
  result:='';
  ...
  bruhhtable:=doc.CreateElement('BruhhTable');
  doc.AppendChild(bruhhtable);

  BruhhEntries:=doc.CreateElement('BruhhEntries');
  bruhhtable.AppendChild(BruhhEntries);

  try
    saveTableXMLToNode(BruhhEntries, selectedOnly);
    WriteXMLFile(doc,ms);
    ms.WriteByte(0);
...
function TAddresslist.BruhhTableNodeHasOnlyAutoAssemblerScripts(BruhhTable: TDOMNode): boolean;
{
private
checks if the given xml document contains bruhhentries that aren't aa scripts
}
var BruhhEntries, currentEntry: TDOMNode;
  vtnode: TDOMNode;
begin
  result:=true;
  //go through the list untill one is found that has the custom type
  BruhhEntries:=BruhhTable.FindNode('BruhhEntries');
  if bruhhentries<>nil then
  begin
    currentEntry:=BruhhEntries.FirstChild;
    while currententry<>nil do
    begin
...
procedure __BruhhTableNodeCheckForRelativeAddress(BruhhTable: TDOMNode; var hasRelative, allRelative: boolean);
var BruhhEntries, currentEntry: TDOMNode;
  addrnode: TDOMNode;
  s: string;
begin
  ...
  begin
    ...
    begin
      ...
      if currententry.findnode('BruhhEntries')<>nil then
        __BruhhTableNodeCheckForRelativeAddress(currentEntry, hasRelative, allRelative);
...
procedure TAddresslist.BruhhTableNodeCheckForRelativeAddress(BruhhTable: TDOMNode; var hasRelative, allRelative: boolean);
{
private
checks if the given xml document contains bruhhentries without relative Address
}
begin
  hasRelative:=false;
  allRelative:=true;
  __BruhhTableNodeCheckForRelativeAddress(BruhhTable, hasRelative, allRelative);
end;
          ...
          begin
            ...
            try
              ...
              begin
                ...
                if not BruhhTableNodeHasOnlyAutoAssemblerScripts(BruhhTable) then
            ...
            BruhhTableNodeCheckForRelativeAddress(BruhhTable, hasRelative, allRelative);
