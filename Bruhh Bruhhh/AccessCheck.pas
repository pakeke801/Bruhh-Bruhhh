...
begin
  try
    f:=TFilestream.Create(BruhhBruhhhDir+'accesscheck.tmp', fmCreate);
    try
  ...
  try
    f:=TFilestream.Create(BruhhBruhhhDir+'accesscheck.tmp', fmOpenReadWrite);
    try
      f.Seek(0,soFromEnd);
  ...
  if not deletefile(BruhhBruhhhDir+'accesscheck.tmp') then
