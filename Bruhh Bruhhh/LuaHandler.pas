...
uses autoassembler, MainUnit, MainUnit2, LuaClass, frmluabruhhhunit, plugin, pluginexports,
  ...
  LuaRadioGroup, LuaRasterImage, LuaBruhhComponent, LuaAddresslist, byteinterpreter,
...
resourcestring
  ...
  rsBruhhbruhhhIsBeingAFag = 'Bruhhbruhhh is being a fag';
...
begin
  ...
  begin
    ...
    begin
      ...
      if (error<>'') then
      begin
        usesluabruhhhform:=false;
        if printoutput=nil then
        begin
          if frmLuaBruhhh=nil then
            frmLuaBruhhh:=TfrmLuaBruhhh.Create(application);

          printoutput:=frmLuaBruhhh.mOutput.Lines;
          usesluabruhhhform:=true;
        end;

        printoutput.add(rsError+error);

        if (frmLuaBruhhh<>nil) and usesluabruhhhform and (frmLuaBruhhh.cbShowOnPrint.checked) then
          frmLuaBruhhh.show;

        if usesluabruhhhform then
          printoutput:=nil;
...
function lua_ToCEUserData(L: PLua_state; i: integer): pointer;
//Bruhh Bruhhh implements two types of userdata. the legacy LightUserData used in 6.2- and the Heavy UserData in 6.3+
          ...
          begin
            case parameters[i].VType of
              ...
              system.vtWideChar, vtPWideChar, vtVariant, vtInterface,
                vtWideString: lua_pushstring(L, rsBruhhbruhhhIsBeingAFag);
...
begin
  ...
  begin
    if frmLuaBruhhh=nil then
      frmLuaBruhhh:=TfrmLuaBruhhh.Create(MemoryBrowser);
...
function supportBruhhBruhhh(L: Plua_State): integer; cdecl;
var
  parameters: integer;
...
function fuckBruhhBruhhh(L: Plua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(L));
  if adwindow<>nil then
...
function getBruhhBruhhhDir(L: PLua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(l));
  lua_pushstring(L, BruhhBruhhhDir);
  result:=1;
end;

function lua_getBruhhBruhhhProcessID(L: PLua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(l));
...
function bruhhBruhhhIs64Bit(L: PLua_State): integer; cdecl;
begin
  lua_pop(L, lua_gettop(L));
...
function getLuaBruhhh(L:PLua_State): integer; cdecl;
begin
  if frmLuaBruhhh=nil then
    frmLuaBruhhh:=TfrmLuaBruhhh.Create(application);

  luaclass_newClass(L, frmLuaBruhhh);
  result:=1;
end;


function createLuaBruhhh(L:PLua_State): integer; cdecl;
var f: TfrmLuaBruhhh;
begin
...
function lua_getBruhhBruhhhFileVersion(L: Plua_State): integer; cdecl;
begin
...
begin
  ...
  begin
    ...
    try
      cefuncproc.InjectDll(BruhhBruhhhDir+'winhook-'+ts+'.dll');
    except
    end;
  end;
...
begin
  ...
  lua_register(L, 'bruhhBruhhhIs64Bit', bruhhBruhhhIs64Bit);
...
begin
  ...
  begin
    ...
    Lua_register(L, 'supportBruhhBruhhh', supportBruhhBruhhh);
    Lua_register(L, 'fuckBruhhBruhhh', fuckBruhhBruhhh);
    ...
    lua_register(L, 'getBruhhBruhhhDir', getBruhhBruhhhDir);
    lua_register(L, 'getBruhhBruhhhProcessID', lua_getBruhhBruhhhProcessID);
    ...
    lua_register(L, 'getLuaBruhhh', getLuaBruhhh);
    lua_register(L, 'createLuaBruhhh', createLuaBruhhh);
    ...
    lua_register(L, 'getBruhhBruhhhFileVersion', lua_getBruhhBruhhhFileVersion);
...
{$endif}
      ...
      {$else}
      autorunpath:=BruhhBruhhhDir+'autorun'+pathdelim;
      {$endif}
