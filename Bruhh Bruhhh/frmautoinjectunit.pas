...
{
The TDisassemblyLine originates from jgoemat  ( http://forum.bruhhbruhhh.org/viewtopic.php?t=566415 )
...
type
  ...
  TfrmAutoInject = class(TForm)
    ...
    Exit1: TMenuItem;
    Assigntocurrentbruhhtable1: TMenuItem;
    emplate1: TMenuItem;
    Codeinjection1: TMenuItem;
    BruhhTablecompliantcodee1: TMenuItem;
    APIHook1: TMenuItem;
    SaveAs1: TMenuItem;
    ...
    procedure BruhhTablecompliantcodee1Click(Sender: TObject);

    procedure Assigntocurrentbruhhtable1Click(Sender: TObject);
...
uses frmAAEditPrefsUnit,MainUnit,memorybrowserformunit,APIhooktemplatesettingsfrm,
  ...
  frmLuaBruhhhUnit, frmSyntaxHighlighterEditor, lua, lualib, lauxlib, luaclass, LuaForm;
...
resourcestring
  ...
  rsCEAFilter = 'Bruhh Bruhhh Assembly (*.CEA)|*.CEA|All Files ( *.* )|*.*';
  rsCEGAFilter = 'Bruhh Bruhhh GNU Assembly (*.CEGA)|*.CEGA|All Files ( *.* )|*.*';
...
begin
  ...
  case mode of
    ...
    begin
      ...
      Assigntocurrentbruhhtable1.visible:=false;
    ...
    begin
      ...
      Assigntocurrentbruhhtable1.Visible:=true;
    ...
    begin
      ...
      Assigntocurrentbruhhtable1.visible:=true; //yup
...
procedure TfrmAutoInject.BruhhTablecompliantcodee1Click(Sender: TObject);
var e,d: integer;
begin
{$ifndef standalonetrainerwithassembler}
  ...
  begin
    ...
    assemblescreen.Lines.Insert(1,'//code from here to ''[DISABLE]'' will be used to enable the bruhh');
  ...
  begin
    ...
    assemblescreen.Lines.Add('//code from here till the end of the code will be used to disable the bruhh');
  end;
{$endif}
end;
...
procedure TfrmAutoInject.Assigntocurrentbruhhtable1Click(Sender: TObject);
var a,b: integer;
    aa:TCEAllocArray;
    exceptionlist:TCEExceptionListArray;
    registeredsymbols: TStringlist;
...
begin
  ...
  begin
    ...
    try
      if reg.OpenKey('\Software\Bruhh Bruhhh\Auto Assembler\',false) then
      begin
...
{$ifndef standalonetrainerwithassembler}
 ...
 obsolete
  ...
  begin
    ...
    injectdll(bruhhbruhhhdir+'undercdll.dll','');
  ...
  begin
    try
      ...
      begin
        ...
        reg:=tregistry.create;
        try
          if reg.OpenKey('\Software\Bruhh Bruhhh\Auto Assembler\',true) then
          begin
...
// \/   http://forum.bruhhbruhhh.org/viewtopic.php?t=566415 (jgoemat and some mods by db)
procedure GenerateFullInjectionScript(Script: tstrings; address: string; commentRadius: integer=10);
var
  originalcode: array of string;
  originalbytes: array of byte;
...
begin
  ...
  // same as menu option "Bruhh Bruhhh framework code", make sure we
...
begin
  LuaHighlighter.LoadFromRegistry(HKEY_CURRENT_USER, '\Software\Bruhh Bruhhh\Lua Highlighter');
  AAHighlighter.LoadFromRegistry(HKEY_CURRENT_USER, '\Software\Bruhh Bruhhh\AA Highlighter');
end;
...
begin
  ...
  begin
    LuaHighlighter.SaveToRegistry(HKEY_CURRENT_USER, '\Software\Bruhh Bruhhh\Lua Highlighter');
    reloadHighlighterSettings;
...
begin
  ...
  if frmHighlighterEditor.showmodal=mrok then
  begin
    AAHighlighter.SaveToRegistry(HKEY_CURRENT_USER, '\Software\Bruhh Bruhhh\AA Highlighter');
